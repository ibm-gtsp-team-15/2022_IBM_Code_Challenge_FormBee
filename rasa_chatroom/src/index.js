		...
		for (let i = -1; i < messages.length; i++) {
			if (i < 0) {
				...
				if (
					...
					for (
						let j = 0;
						j < messageText.length && this.demoIsPlaying;
						j++
					) {
						const currentMessageText = messageText.substring(
							0,
							j + 1
						);
				...
				if (
					...
					this.render(
						messages.slice(0, i).concat({
							...
							message: {
								type: "button",
								buttons: currentMessageContent.buttons.map(
									(b) => ({
										title: b.title,
										payload: b.payload,
									})
